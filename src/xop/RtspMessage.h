...
public:
	enum Method
	{
		OPTIONS=0, DESCRIBE, ANNOUNCE, SETUP, PLAY, PAUSE,  TEARDOWN,  GET_PARAMETER,  RECORD,
		RTCP, NONE,
	};

	const char* MethodToString[11] =
	{
		"OPTIONS", "DESCRIBE", "ANNOUNCE", "SETUP", "PLAY", "PAUSE", "TEARDOWN", "GET_PARAMETER", "RECORD",
		"RTCP", "NONE"
	};
	...
	int BuildRecordRes(const char* buf, int buf_size, uint32_t session_id);
	...
	int BuildSessionExistRes(const char* buf, int buf_size);
...
public:
	enum Method
	{
		OPTIONS=0, DESCRIBE, ANNOUNCE, SETUP, PLAY, RECORD, RTCP,
		NONE, 
	};
