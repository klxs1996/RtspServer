...
bool RtspRequest::ParseHeadersLine(const char* begin, const char* end)
{
	...
	if (method_ == ANNOUNCE || method_ == RECORD)
	{
		state_ = kGotAll;
		return true;
	}
...
int xop::RtspRequest::BuildRecordRes(const char * buf, int buf_size, uint32_t session_id)
{
	memset((void*)buf, 0, buf_size);
	snprintf((char*)buf, buf_size,
		"RTSP/1.0 200 OK\r\n"
...
int xop::RtspRequest::BuildSessionExistRes(const char * buf, int buf_size)
{
	memset((void*)buf, 0, buf_size);
	snprintf((char*)buf, buf_size,
